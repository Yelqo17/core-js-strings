...
function isPalindrome(str) {
  const strReplaced = str.toLowerCase().replaceAll(/[^a-zA-Z]/g, '');
  for (let i = 0; i < strReplaced.length / 2; i += 1) {
    if (strReplaced[i] !== strReplaced[strReplaced.length - i - 1]) {
      return false;
    }
  }
  return true;
...
function reverseWords(str) {
  ...
  return reversedArr.join(' ').toString();
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (code >= 97 && code <= 122) {
      res += String.fromCharCode(((code - 97 + 13) % 26) + 97);
    } else if (code >= 65 && code <= 90) {
      res += String.fromCharCode(((code - 65 + 13) % 26) + 65);
    } else {
